...
import { ICategory, IProduct, IService } from '../../../types/types'
...
const ServiceForm = () => {
  ...
  const getProductsData = async(id)=>{
      const {data}= await instance.get(`service/all/${id}`)
      setProducts(data)
  }
  ...
  const [selectedProduct, setSelectedProduct ] = useState<IService>()
  ...
  const addHandler = (e)=>{
   ...
   console.log(data)
    multiInstance.post('service', data).then(()=>{
  ...
  const updHandler = (e)=>{
    ...
    multiInstance.patch(`service/${selectedProduct?.id}`, data).then(()=>{
  ...
  const deleteHandler = (e)=>{
    ...
    multiInstance.delete(`service/${selectedProduct?.id}`).then(()=>{
